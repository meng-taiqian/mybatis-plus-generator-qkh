...
//                tableInfo.setImportPackages("com.baomidou.mybatisplus.annotations.TableName");
            ...
            ctx.put("instance", tableInfo.getInstanceName());
    ...
    private void outPutBaseClasses() {
        if (null == injectionConfig || null == injectionConfig.getMap())
            return;
        Map cfg = injectionConfig.getMap();
        VelocityContext ctx = new VelocityContext();
        ctx.put("package", config.getPackageInfo());
        try {
            if (((Integer)(cfg.get("baseMapper"))).intValue() == 2) {
                vmToFile(ctx, config.getTemplate().getBaseMapper(), config.getPathInfo().get("mapper_path") + File.separator + "BaseMapper" + ConstVal.JAVA_SUFFIX);
                vmToFile(ctx, config.getTemplate().getiService(), config.getPathInfo().get("service_path") + File.separator + "IService" + ConstVal.JAVA_SUFFIX);
                vmToFile(ctx, config.getTemplate().getiServiceImpl(), config.getPathInfo().get("serviceimpl_path") + File.separator + "ServiceImpl" + ConstVal.JAVA_SUFFIX);
            }
        } catch (IOException e) {
            e.printStackTrace();
        }
